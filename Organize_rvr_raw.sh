...
WD='/home/despoB/kaihwang/TRSE/TRSEPPI/RVR'
rvr_raw='/home/despo/espool/rvr'


## organize rvr's 
#subjects: 106 107 108 109 110 111 112 113 114 115 116 117 118
for s in 903 905 906 910 912 ; do
	...
	# mkdir ${WD}/${s}/Raw
	# #ln -s ${rvr_Raw}/${s}/PRE/*.tgz ${WD}/${s}/Raw/Raw.tgz

	# fi

	# if [ ! -e ${WD}/${s}/Raw/Raw.tgz ]; then
	# 	cp ${rvr_raw}/${s}*/*.tgz ${WD}/${s}/Raw/Raw.tgz
	...
	cp ${rvr_raw}/${s}/SourceDICOMs/MPRAGE*/* ${WD}/${s}/MPRAGE

	# get BOLD organized

	cd ${rvr_raw}/${s}/SourceDICOMs
	...
	for run_dir in $(ls -d *TRSE* | sort -V); do
		...
		#	3dcopy ${rvr_raw}/${s}/PRE/epi/ep2d_neuro_64_TR1000_TRSEblock_${i}.nii.gz ${WD}/${s}/run${i}/run${i}_raw.nii.gz 
		#fi	
